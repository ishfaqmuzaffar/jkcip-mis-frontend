...
import { formatDate, getBeneficiaryStatusMeta, DISTRICTS_JK, cn } from "@/lib/utils";
          ...
          <tbody>
            {isLoading ? (
              ...
              filtered.map((b) => {
                ...
                const appStatus = (b as any).applicationStatus;
                const category = (b as any).category;
                return (
                  <tr key={b.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="table-td">
                      <select
                        ...
                        value={b.status}
                        onChange={(e) =>
                          statusUpdateMutation.mutate({ id: b.id, status: e.target.value })
                        }
                        disabled={!canWrite}
                      >
                    ...
                    <td className="table-td">
                      {category ? (
                        <span className={cn("badge", {
                          "bg-blue-100 text-blue-700": category === "SC",
                          "bg-green-100 text-green-700": category === "ST",
                          "bg-purple-100 text-purple-700": category === "OBC",
                          "bg-amber-100 text-amber-700": category === "PHH",
                          "bg-slate-100 text-slate-600": category === "GENERAL",
                        })}>
                          {category}
                        </span>
                      ) : "—"}
                    ...
                    <td className="table-td">
                      {appStatus ? (
                        <span className={cn("badge", {
                          "bg-green-100 text-green-700": appStatus === "APPROVED",
                          "bg-amber-100 text-amber-700": appStatus === "PENDING",
                          "bg-red-100 text-red-700": appStatus === "REVERTED",
                        })}>
                          {appStatus}
                        </span>
                      ) : "—"}
                    ...
                    <td className="table-td text-xs text-slate-600">
                      {(b as any).scheme?.title ?? (b as any).project?.name ?? "—"}
                    </td>
...
function BeneficiaryDialog({ beneficiary, schemes, projects, onClose }: any) {
  ...
  const [form, setForm] = useState<any>({
    ...
    projectId: beneficiary?.projectId ?? undefined,
  });

  const mutation = useMutation({
    mutationFn: () =>
      beneficiary
        ? beneficiariesApi.update(beneficiary.id, form)
        : beneficiariesApi.create(form),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["beneficiaries"] });
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                value={form.schemeId ?? ""}
                onChange={(e) =>
                  setForm({ ...form, schemeId: e.target.value ? Number(e.target.value) : undefined })
                }
              >
        ...
        <div className="flex justify-end gap-3 px-5 py-4 border-t border-slate-100">
          ...
          >
            {mutation.isPending ? "Saving…" : (
              <><Save className="w-3.5 h-3.5" /> {beneficiary ? "Update" : "Add"}</>
            )}
          </button>
        </div>
      </div>
